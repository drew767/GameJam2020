...
public class WeaponScript : MonoBehaviour {
	...
	public WeaponStyles currentStyle;
	[HideInInspector]
	public MouseLookController mls;
	...
	void Awake(){
		...
		mls = GameObject.FindGameObjectWithTag("Player").GetComponent<MouseLookController>();
	...
	*/
	...
	void CrossHairExpansionWhenWalking(){
		...
		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && Input.GetAxis("Fire1") == 0){//ifnot shooting
			...
			if(player.GetComponent<PlayerController>().maxSpeed < runningSpeed){ //not running
		...
		if (Input.GetAxis ("Vertical") > 0 && Input.GetAxisRaw ("Fire2") == 0 && meeleAttack == false && Input.GetAxisRaw ("Fire1") == 0) {
			if (Input.GetKeyDown (KeyCode.LeftShift)) {
				if (pmS.maxSpeed == walkingSpeed) {
					//TODO: implement max speed change
	...
	*/
	...
	void Shooting(){
		...
		if (!meeleAttack) {
			if (currentStyle == WeaponStyles.nonautomatic) {
				if (Input.GetButtonDown ("Fire1")) {
					ShootMethod ();
				}
			}
			if (currentStyle == WeaponStyles.automatic) {
